...
def plot(
    ...
    pl = pv.Plotter(shape=(4, 1), notebook=True, window_size=(1024, 2304))
    ...
    pl.add_mesh(mesh_displacements, color="black", style="wireframe", line_width=2)
    pl.add_mesh(
        warped_mesh,
    ...
    pl.show_axes()
    pl.enable_2d_style()
    pl.add_title("Displacements (scaled by factor of " + str(scale_displacements) + ")")

    # plot stresses
    mesh_stresses = get_stresses_mesh(problem, nodal_displacements, use_pian_sumihara)
    for i, stress_name in enumerate(["xx", "yy", "xy"]):
        pl.subplot(i + 1, 0)
        pl.add_mesh(mesh_stresses, color="black", style="wireframe", line_width=2)
        pl.add_mesh(
            mesh_stresses,
        ...
        pl.show_axes()
        pl.enable_2d_style()
        pl.add_title("Stress " + stress_name)
    pl.show()
